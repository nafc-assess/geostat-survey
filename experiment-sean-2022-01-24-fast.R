...
pop <- sim_abundance(
  ...
  sim_distribution(
    grid = make_grid(
      ...
      shelf_width = 100, # Approximate width of the shelf in km
...
survey <- sim_survey(
  ...
  set_den = 7 / 1000, # set per strata ~ strata area with a set density rule, sample size = sum(set numbers per strat) * years
...
#   mesh = mesh_present,
#   time = "year",
#   family = sdmTMB::truncated_nbinom2(link = "log"),
#   control = ctl,
#   spatial = 'off',
#   spatiotemporal = "off",
#   silent = FALSE
...
# m2_tnb
# library(glmmTMB)
# m2_tnb_glmmtmb <- glmmTMB(N ~ 0 + as.factor(year),
#   data = dat_present,
#   family = glmmTMB::truncated_poisson(link = "log")
# )
# summary(m2_tnb_glmmtmb)
...
grid_dat$offset <- 0
grid_dat <- left_join(grid_dat, select(xy_sim, x, y, depth))
