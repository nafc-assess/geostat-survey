# #############  Packages
#
# library(SimSurvey)
# library(sdmTMB)
# library(tidyr)
# library(future)
# library(purrr)
# library(tictoc)
# library(ggplot2)
# library(data.table)
# library(dplyr)
#
#
# plan(multisession, workers = floor(availableCores()/2))
#
# #############  Population
#
# set.seed(1)
#
# population <- function(iter) {
#   set.seed(iter * 10)
#   pop <- sim_abundance(ages = 1:20,
#                        years = 1:20,
#                        Z = sim_Z(log_mean = log(0.63),
#                                  log_sd = 0.3,
#                                  phi_age = 0.9,
#                                  phi_year = 0.5,
#                                  plot = FALSE),
#                        R = sim_R(log_mean = log(75e+06),
#                                  log_sd = 0.5,
#                                  random_walk = TRUE,
#                                  plot = FALSE),
#                        N0 = sim_N0(N0 = "exp",
#                                    plot = FALSE),
#                        growth = sim_vonB(Linf = 120,
#                                          L0 = 5,
#                                          K = 0.11,
#                                          log_sd = 0.15,
#                                          plot = FALSE,
#                                          length_group = 3))
# }
#
# # tic()
# # purrr::map(seq_len(6), population)
# # toc()
#
# tic()
# pop <- furrr::future_map(seq_len(6), population, .options = furrr::furrr_options(seed = TRUE, packages = "SimSurvey"))
# toc()
#
# ############# Population distribution
#
# distribution <- function(x) {
#   sim_distribution(x,
#                    grid = make_grid(x_range = c(-150, 150),
#                                     y_range = c(-150, 150),
#                                     res = c(10, 10),
#                                     shelf_depth = 200,
#                                     shelf_width = 100,
#                                     depth_range = c(0, 1000),
#                                     n_div = 1,
#                                     strat_breaks = seq(0, 1000, by = 40),
#                                     strat_splits = 2,
#                                     method = "spline"),
#                    ays_covar = sim_ays_covar(sd = 2.5,
#                                              range = 200,
#                                              phi_age = 0.5,
#                                              phi_year = 0.9,
#                                              group_ages = 12:20),
#                    depth_par = sim_parabola(mu = log(80),
#                                             sigma = 0.25, plot=FALSE, log_space = TRUE))
# }
#
# # tic()
# # purrr::map(pop, distribution)
# # toc()
#
# tic()
# sim <- furrr::future_map(pop, distribution, .options = furrr::furrr_options(seed = TRUE, packages = "SimSurvey"))
# toc()
#
# ############# Survey
#
# survey <-  function(x) {
#   sim_survey(x,
#              n_sims = 1,
#              q = sim_logistic(k = 2, x0 = 3, plot = FALSE),
#              trawl_dim = c(1.5, 0.02),
#              resample_cells = FALSE,
#              binom_error = TRUE,
#              min_sets = 2,
#              set_den = 2/1000)
# }
#
# # tic()
# # purrr::map(sim, survey)
# # toc()
#
# tic()
# survey <- furrr::future_map(sim, survey, .options = furrr::furrr_options(seed = TRUE, packages = "SimSurvey"))
# toc()


####### Reduced set

setdet <- map(survey, function(x) {pluck(x, 'setdet')}) ## plucking setdets


#### 20 %

perc20 <- function(x, year_cutoff) {
  a <- x %>%
    filter(.$year >= year_cutoff) %>%
    group_by(sim, year, strat) %>%
    filter(unique(strat_sets) > 2) %>%
    sample_frac(0.8)
  b <- x%>%
    filter(.$year < year_cutoff)
  ab <- rbind(a,b) %>%
    as.data.table()
  return(ab)
}


perc20 <- map(setdet, perc20, year_cutoff=11)


#Checking the sample distribution
d <- design_index_sc2_20[[5]]$setdet %>%
  filter(year == 15) %>%
  group_by(sim, strat, year, strat_cells) %>%
  summarise(setno = n())  %>%
  mutate(ratio = setno/strat_cells)
sum(d$setno)

f <- d %>% filter(strat_cells == 75)
sum(f$setno)



#### 50 %

perc50 <- function(x, year_cutoff) {
  a <- x %>%
    filter(.$year >= year_cutoff) %>%
    group_by(sim, year, strat) %>%
    filter(unique(strat_sets) > 2) %>%
    sample_frac(0.5)
  b <- x%>%
    filter(.$year < year_cutoff)
  ab <- rbind(a,b) %>%
    as.data.table()
  return(ab)
}


perc50 <- map(setdet, perc50, year_cutoff=11)


# ### DEV ### DEV ### DEV
# ### DEV ### DEV ### DEV

# d <- design_index_sc2_50[[3]]$setdet %>%
#   filter(year == 17) %>%
#   group_by(sim, strat, year, strat_cells) %>%
#   summarise(setno = n())  %>%
#   mutate(ratio = setno/strat_cells)
# sum(d$setno)
#
# f <- d %>% filter(strat_cells == 45)
# sum(f$setno)
#
#
# samples <- design_index_sc2_50[[1]]$setdet %>%
#   dplyr::select(year, strat, cell, N, n, strat_area)
#
# dat2 <- dat%>%
#   left_join(., as_tibble(samples), by = c("year", "cell")) %>%
#   na.omit() %>%
#   mutate(sample = "sample")
#
# ggplot(dat) +
#   geom_raster(aes(x, y, fill = log(N))) +
#   facet_wrap(~year) +
#   geom_point(aes(x, y, color=as.factor(sample)), size=1, data= dat2, inherit.aes = FALSE)+
#   scale_color_manual(values=c("#111111"), name = "sample", guide = guide_legend(title = NULL)) +
#   coord_fixed(expand = FALSE)

# ### DEV ### DEV ### DEV# ### DEV ### DEV ### DEV

#############

############# Design-based index

#### 20 %

survey20f <- function(i) {
  survey[[i]]$setdet <- perc20[[i]]
  survey_index <- survey[[i]] %>% run_strat()
}

design_index_sc2_20 <- map(seq_along(survey), survey20f)

design_index_sc2_20 <- map(seq_along(design_index_sc2_20), function(i){
  design_index_sc2_20[[i]]$total_strat %>%
    mutate(N = total, upr =total_ucl, lwr = total_lcl, type = "Design-based") %>%
    dplyr::select(year, N, type, lwr, upr)})

for( i in seq_along(design_index_sc2_20)){
  design_index_sc2_20[[i]]$iter <- as.numeric(i)
  design_index_sc2_20[[i]]$true <- true_index[[i]]$N
}

design_index_sc2_20_b <- as.data.frame(do.call(rbind, design_index_sc2_20))
design_index_sc2_20_b$scenario <- "2M"


#### 50 %

survey50f <- function(i) {
  survey[[i]]$setdet <- perc50[[i]]
  survey_index <- survey[[i]] %>% run_strat()
}

design_index_sc2_50 <- map(seq_along(survey), survey50f)

design_index_sc2_50 <- map(seq_along(design_index_sc2_50), function(i){
  design_index_sc2_50[[i]]$total_strat %>%
    mutate(N = total, upr =total_ucl, lwr = total_lcl, type = "Design-based") %>%
    dplyr::select(year, N, type, lwr, upr)})

for( i in seq_along(design_index_sc2_50)){
  design_index_sc2_50[[i]]$iter <- as.numeric(i)
  design_index_sc2_50[[i]]$true <- true_index[[i]]$N
}

design_index_sc2_50_b <- as.data.frame(do.call(rbind, design_index_sc2_50))
design_index_sc2_50_b$scenario <- "2H"

#### Bootstrapped

#### 20 %
tic()
boot_index_sc2_20 <- furrr::future_map(perc20, boot_wrapper, reps=2000, .options = furrr::furrr_options(seed = TRUE))
toc()

for( i in seq_along(boot_index_sc2_20 )){
  boot_index_sc2_20 [[i]]$iter <- as.numeric(i)
}

boot_index_sc2_20_b <- as.data.frame(do.call(rbind, boot_index_sc2_20 ))
boot_index_sc2_20_b$scenario <- "2M"

boot_index_sc2_20_b <- boot_index_sc2_20_b %>% select(year, mean_boot, lwr, upr, type, iter, scenario) %>%
  rename(N=mean_boot)

#### 50 %
tic()
boot_index_sc2_50 <- furrr::future_map(perc50, boot_wrapper, reps=2000, .options = furrr::furrr_options(seed = TRUE))
toc()

for( i in seq_along(boot_index_sc2_50 )){
  boot_index_sc2_50 [[i]]$iter <- as.numeric(i)
}

boot_index_sc2_50_b <- as.data.frame(do.call(rbind, boot_index_sc2_50 ))
boot_index_sc2_50_b$scenario <- "2H"

boot_index_sc2_50_b <- boot_index_sc2_50_b %>% select(year, mean_boot, lwr, upr, type, iter, scenario) %>%
  rename(N  = mean_boot)

#################  sdmTMB

### Functions

sdm_data_fn <- function(x) {

  xy <- as_tibble(x$grid_xy)
  dat <- as_tibble(x$setdet) %>%
    dplyr::select(x, y, set, year, N = n, tow_area) %>%
    left_join(., xy, by = c("x", "y")) %>%
    mutate(offset = log(tow_area), density = N / tow_area)
}

mesh_sdm_fn <- function(sdm_data){
  mesh <- sdmTMB::make_mesh(
    sdm_data,
    xy_cols = c("x", "y"),
    cutoff = 40)
}

sdm_IID_fn <- function(x, y){

  fit_IID <- sdmTMB(N ~ 0 + as.factor(year) + offset,
                    data = x,
                    mesh = y,
                    time = "year",
                    family = nbinom2(link = "log"),
                    spatial = TRUE,
                    spatiotemporal = "IID")
}

sdm_IID_depth_fn <- function(x, y){

  fit_IID_depth <- sdmTMB(N ~  0 + as.factor(year) + offset + s(log(depth), k= 4),
                          data = x,
                          mesh = y,
                          time = "year",
                          family = nbinom2(link = "log"),
                          spatial = TRUE,
                          spatiotemporal = "IID")
}

sdm_AR1_fn <- function(x, y) {

  fit_AR1 <- sdmTMB(N ~ 0 + as.factor(year) + offset,
                    data = x,
                    mesh = y,
                    time = "year",
                    family = nbinom2(link = "log"),
                    spatial = TRUE,
                    spatiotemporal = c("AR1"))
}


sdm_IID_tw_fn <- function(x, y){

  fit_IID_tw <- sdmTMB(density ~ 0 + as.factor(year),
                       data = x,
                       mesh = y,
                       time = "year",
                       family = tweedie(),
                       spatial = TRUE,
                       spatiotemporal = "IID")
}

fit_IID_tw_depth <- function(x, y){

  fit_IID_tw_depth <- sdmTMB(density ~ 0 + as.factor(year) + s(log(depth), k= 4),
                             data = x,
                             mesh = y,
                             time = "year",
                             family = tweedie(),
                             spatial = TRUE,
                             spatiotemporal = "IID")
}

sdm_newdata_fn <- function(survey, sdm_data) {

  grid_dat <- as_tibble(dplyr::select(survey$grid_xy, x, y, depth)) %>% distinct()
  grid_dat <- purrr::map_dfr(sort(unique(sdm_data$year)), ~ bind_cols(grid_dat, year = .))
  grid_dat$offset <- 0
  grid_dat$area <-survey$setdet$cell_area[1]
  return(grid_dat)
}


sdm_prediction_fn <- function(x, y){

  pred <- predict(x,
                  newdata = y,
                  eturn_tmb_object = TRUE,
                  nsim = 200L)
}



sdm_index_IID_fn <- function(x, y){
  index_IID <- get_index_sims(x, area = y$area) %>%
    mutate(type = "IID + NB2", N = est)
}

sdm_index_IID_depth_fn <- function(x, y){
  index_IID <- get_index_sims(x, area = y$area) %>%
    mutate(type = "IID + NB2 + depth", N = est)
}

sdm_index_AR1_fn <- function(x, y){
  index_AR1 <- get_index_sims(x, area = y$area) %>%
    mutate(type = "AR1 + NB2", N = est)
}

sdm_index_IID_tw_fn <- function(x, y){
  index_IID <- get_index_sims(x, area = y$area) %>%
    mutate(type = "IID + TW", N = est)
}

sdm_index_IID_tw_fn_depth <- function(x, y){
  index_IID <- get_index_sims(x, area = y$area) %>%
    mutate(type = "IID + TW + depth", N = est)
}



#### 20 %
survey_sc2_20 <- survey
for( i in seq_along(survey_sc2_20)){
  survey_sc2_20[[i]]$setdet <- perc20[[i]]}

tic()
sdm_data_sc2_20 <- furrr::future_map(survey_sc2_20, sdm_data_fn, .options = furrr::furrr_options(seed = TRUE))
toc()

tic()
mesh_sdm_sc2_20 <- furrr::future_map(sdm_data_sc2_20, mesh_sdm_fn, .options = furrr::furrr_options(seed = TRUE))
toc()

tic()
sdm_IID_sc2_20 <- furrr::future_map2(sdm_data_sc2_20, mesh_sdm_sc2_20, sdm_IID_fn)
toc()

tic()
sdm_IID_depth_sc2_20 <- furrr::future_map2(sdm_data_sc2_20, mesh_sdm_sc2_20, sdm_IID_depth_fn)
toc()

tic()
sdm_AR1_sc2_20 <- furrr::future_map2(sdm_data_sc2_20, mesh_sdm_sc2_20, sdm_AR1_fn)
toc()

tic()
sdm_tw_sc2_20 <- furrr::future_map2(sdm_data_sc2_20, mesh_sdm_sc2_20, sdm_IID_tw_fn)
toc()


tic()
sdm_tw_depth_sc2_20 <- furrr::future_map2(sdm_data_sc2_20, mesh_sdm_sc2_20, fit_IID_tw_depth)
toc()

tic()
sdm_newdata_sc2_20 <- furrr::future_map2(survey, sdm_data_sc2_20, sdm_newdata_fn)
toc()


tic()
sdm_prediction_IID_sc2_20 <- furrr::future_map2(sdm_IID_sc2_20, sdm_newdata_sc2_20, sdm_prediction)
toc()

tic()
sdm_prediction_IID_depth_sc2_20 <- furrr::future_map2(sdm_IID_depth_sc2_20, sdm_newdata_sc2_20, sdm_prediction)
toc()

tic()
sdm_prediction_AR1_sc2_20 <- furrr::future_map2(sdm_AR1_sc2_20, sdm_newdata_sc2_20, sdm_prediction)
toc()

tic()
sdm_prediction_tw_sc2_20 <- furrr::future_map2(sdm_tw_sc2_20, sdm_newdata_sc2_20, sdm_prediction)
toc()

tic()
sdm_prediction_tw_depth_sc2_20 <- furrr::future_map2(sdm_tw_depth_sc2_20, sdm_newdata_sc2_20, sdm_prediction)
toc()




tic()
sdm_index_IID_sc2_20 <- furrr::future_map(sdm_prediction_IID_sc2_20, sdm_index_IID_fn)
toc()

for( i in seq_along(sdm_index_IID_sc2_20)){
  sdm_index_IID_sc2_20[[i]]$iter <- as.numeric(i)
  sdm_index_IID_sc2_20[[i]]$true <- true_index[[i]]$N
}
sdm_index_IID_2_sc2_20 <- as.data.frame(do.call(rbind, sdm_index_IID_sc2_20))
sdm_index_IID_2_sc2_20$scenario <- "2M"


tic()
sdm_index_IID__depth_sc2_20 <- furrr::future_map(sdm_prediction_IID_sc2_20, sdm_index_IID_depth_fn)
toc()

for( i in seq_along(sdm_index_IID__depth_sc2_20)){
  sdm_index_IID__depth_sc2_20[[i]]$iter <- as.numeric(i)
  sdm_index_IID__depth_sc2_20[[i]]$true <- true_index[[i]]$N
}
sdm_index_IID_depth_sc2_20_2 <- as.data.frame(do.call(rbind, sdm_index_IID_sc2_20))
sdm_index_IID_depth_sc2_20_2$scenario <- "2M"


tic()
sdm_index_AR1_sc2_20 <- furrr::future_map(sdm_prediction_AR1_sc2_20, sdm_index_AR1_fn)
toc()

for( i in seq_along(sdm_index_AR1_sc2_20)){
  sdm_index_AR1_sc2_20[[i]]$iter <- as.numeric(i)
  sdm_index_AR1_sc2_20[[i]]$true <- true_index[[i]]$N
}
sdm_index_AR1_2_sc2_20 <- as.data.frame(do.call(rbind, sdm_index_AR1_sc2_20))
sdm_index_AR1_2_sc2_20$scenario <- "2M"


tic()
sdm_index_tw_sc2_20 <- furrr::future_map(sdm_prediction_AR1_sc2_20, sdm_index_IID_tw_fn)
toc()

for( i in seq_along(sdm_index_tw_sc2_20)){
  sdm_index_tw_sc2_20[[i]]$iter <- as.numeric(i)
  sdm_index_tw_sc2_20[[i]]$true <- true_index[[i]]$N
}
sdm_index_tw_2_sc2_20 <- as.data.frame(do.call(rbind, sdm_index_tw_sc2_20))
sdm_index_tw_2_sc2_20$scenario <- "2M"



tic()
sdm_index_tw_depth_sc2_20 <- furrr::future_map(sdm_prediction_AR1_sc2_20, sdm_index_IID_tw_fn_depth)
toc()

for( i in seq_along(sdm_index_tw_depth_sc2_20)){
  sdm_index_tw_depth_sc2_20[[i]]$iter <- as.numeric(i)
  sdm_index_tw_depth_sc2_20[[i]]$true <- true_index[[i]]$N
}
sdm_index_tw_depth_2_sc2_20 <- as.data.frame(do.call(rbind, sdm_index_tw_depth_sc2_20))
sdm_index_tw_depth_2_sc2_20$scenario <- "2M"


#### 50 %

survey_sc2_50 <- survey
for( i in seq_along(survey_sc2_50)){
  survey_sc2_50[[i]]$setdet <- perc50[[i]]}

tic()
sdm_data_sc2_50 <- furrr::future_map(survey_sc2_50, sdm_data_fn, .options = furrr::furrr_options(seed = TRUE))
toc()

tic()
mesh_sdm_sc2_50 <- furrr::future_map(sdm_data_sc2_50, mesh_sdm_fn, .options = furrr::furrr_options(seed = TRUE))
toc()

tic()
sdm_IID_sc2_50 <- furrr::future_map2(sdm_data_sc2_50, mesh_sdm_sc2_50, sdm_IID_fn)
toc()

tic()
sdm_IID_depth_sc2_50 <- furrr::future_map2(sdm_data_sc2_50, mesh_sdm_sc2_50, sdm_IID_depth_fn)
toc()

tic()
sdm_AR1_sc2_50 <- furrr::future_map2(sdm_data_sc2_50, mesh_sdm_sc2_50, sdm_AR1_fn)
toc()

tic()
sdm_tw_sc2_50 <- furrr::future_map2(sdm_data_sc2_50, mesh_sdm_sc2_50, sdm_IID_tw_fn)
toc()


tic()
sdm_tw_depth_sc2_50 <- furrr::future_map2(sdm_data_sc2_50, mesh_sdm_sc2_50, fit_IID_tw_depth)
toc()

tic()
sdm_newdata_sc2_50 <- furrr::future_map2(survey, sdm_data_sc2_50, sdm_newdata_fn)
toc()


tic()
sdm_prediction_IID_sc2_50 <- furrr::future_map2(sdm_IID_sc2_50, sdm_newdata_sc2_50, sdm_prediction)
toc()

tic()
sdm_prediction_IID_depth_sc2_50 <- furrr::future_map2(sdm_IID_depth_sc2_50, sdm_newdata_sc2_50, sdm_prediction)
toc()

tic()
sdm_prediction_AR1_sc2_50 <- furrr::future_map2(sdm_AR1_sc2_50, sdm_newdata_sc2_50, sdm_prediction)
toc()

tic()
sdm_prediction_tw_sc2_50 <- furrr::future_map2(sdm_tw_sc2_50, sdm_newdata_sc2_50, sdm_prediction)
toc()

tic()
sdm_prediction_tw_depth_sc2_50 <- furrr::future_map2(sdm_tw_depth_sc2_50, sdm_newdata_sc2_50, sdm_prediction)
toc()




tic()
sdm_index_IID_sc2_50 <- furrr::future_map(sdm_prediction_IID_sc2_50, sdm_index_IID_fn)
toc()

for( i in seq_along(sdm_index_IID_sc2_50)){
  sdm_index_IID_sc2_50[[i]]$iter <- as.numeric(i)
  sdm_index_IID_sc2_50[[i]]$true <- true_index[[i]]$N
}
sdm_index_IID_2_sc2_50 <- as.data.frame(do.call(rbind, sdm_index_IID_sc2_50))
sdm_index_IID_2_sc2_50$scenario <- "2H"


tic()
sdm_index_IID__depth_sc2_50 <- furrr::future_map(sdm_prediction_IID_sc2_50, sdm_index_IID_depth_fn)
toc()

for( i in seq_along(sdm_index_IID__depth_sc2_50)){
  sdm_index_IID__depth_sc2_50[[i]]$iter <- as.numeric(i)
  sdm_index_IID__depth_sc2_50[[i]]$true <- true_index[[i]]$N
}
sdm_index_IID_depth_sc2_50_2 <- as.data.frame(do.call(rbind, sdm_index_IID_sc2_50))
sdm_index_IID_depth_sc2_50_2$scenario <- "2H"


tic()
sdm_index_AR1_sc2_50 <- furrr::future_map(sdm_prediction_AR1_sc2_50, sdm_index_AR1_fn)
toc()

for( i in seq_along(sdm_index_AR1_sc2_50)){
  sdm_index_AR1_sc2_50[[i]]$iter <- as.numeric(i)
  sdm_index_AR1_sc2_50[[i]]$true <- true_index[[i]]$N
}
sdm_index_AR1_2_sc2_50 <- as.data.frame(do.call(rbind, sdm_index_AR1_sc2_50))
sdm_index_AR1_2_sc2_50$scenario <- "2H"


tic()
sdm_index_tw_sc2_50 <- furrr::future_map(sdm_prediction_AR1_sc2_50, sdm_index_IID_tw_fn)
toc()

for( i in seq_along(sdm_index_tw_sc2_50)){
  sdm_index_tw_sc2_50[[i]]$iter <- as.numeric(i)
  sdm_index_tw_sc2_50[[i]]$true <- true_index[[i]]$N
}
sdm_index_tw_2_sc2_50 <- as.data.frame(do.call(rbind, sdm_index_tw_sc2_50))
sdm_index_tw_2_sc2_50$scenario <- "2H"



tic()
sdm_index_tw_depth_sc2_50 <- furrr::future_map(sdm_prediction_AR1_sc2_50, sdm_index_IID_tw_fn_depth)
toc()

for( i in seq_along(sdm_index_tw_depth_sc2_50)){
  sdm_index_tw_depth_sc2_50[[i]]$iter <- as.numeric(i)
  sdm_index_tw_depth_sc2_50[[i]]$true <- true_index[[i]]$N
}
sdm_index_tw_depth_2_sc2_50 <- as.data.frame(do.call(rbind, sdm_index_tw_depth_sc2_50))
sdm_index_tw_depth_2_sc2_50$scenario <- "2H"


#############
str(true_index_cod)

str(design_index_sc2_20_b)
str(design_index_sc2_50_b)
str(boot_index_sc2_20_b)
str(boot_index_sc2_50_b)
str(sdm_index_IID_2_sc2_20)
str(sdm_index_IID_2_sc2_50)
str(sdm_index_IID_depth_sc2_20_2)
str(sdm_index_IID_depth_sc2_50_2)
str(sdm_index_AR1_2_sc2_20)
str(sdm_index_AR1_2_sc2_50)
str(sdm_index_tw_2_sc2_20)
str(sdm_index_tw_2_sc2_50)
str(sdm_index_tw_depth_2_sc2_20)
str(sdm_index_tw_depth_2_sc2_50)


result_scenario2 <- bind_rows(design_index_sc2_20_b,
                              design_index_sc2_50_b,
                              boot_index_sc2_20_b,
                              boot_index_sc2_50_b,
                              sdm_index_IID_2_sc2_20,
                              sdm_index_IID_2_sc2_50,
                              sdm_index_IID_depth_sc2_20_2,
                              sdm_index_IID_depth_sc2_50_2,
                              sdm_index_AR1_2_sc2_20,
                              sdm_index_AR1_2_sc2_50,
                              sdm_index_tw_2_sc2_20,
                              sdm_index_tw_2_sc2_50,
                              sdm_index_tw_depth_2_sc2_20,
                              sdm_index_tw_depth_2_sc2_50)

### Medium intensity

result_scenario2 %>%
  filter(scenario == "2M")%>%
  ggplot(aes(year, N, group = type)) +
  geom_line(aes(colour = type), size=1) +
  facet_grid(iter~factor(type, levels=c('Design-based','Bootstrapped','IID','AR1')),  scales = "free_y")+
  geom_line(aes(year, N), size=1, data= true_index_cod, inherit.aes = FALSE) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3)+
  labs(x = "Year", y = "N", colour = "type", fill = "type", size = "type")+
  #facet_wrap(~iter)+
  theme_minimal()+
  theme(legend.position = "none")


### High intensity

result_scenario2 %>%
  filter(scenario == "2H")%>%
  ggplot(aes(year, N, group = type)) +
  geom_line(aes(colour = type), size=1) +
  facet_grid(iter~factor(type, levels=c('Design-based','Bootstrapped','IID','AR1')),  scales = "free_y")+
  geom_line(aes(year, N), size=1, data= true_index_cod, inherit.aes = FALSE) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3)+
  labs(x = "Year", y = "N", colour = "type", fill = "type", size = "type")+
  #facet_wrap(~iter)+
  theme_minimal()+
  theme(legend.position = "none")
